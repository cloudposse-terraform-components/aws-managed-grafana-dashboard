...
The `config_input` variable accepts a map of string replacements. These are applied using OpenTofu's `templatestring()` function,
...
| <a name="input_dashboard_file"></a> [dashboard\_file](#input\_dashboard\_file) | Path to a local dashboard JSON file relative to the component's dashboards directory. Either this or dashboard\_url must be set. | `string` | `""` | no |
